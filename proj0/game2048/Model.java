...
public class Model extends Observable {
    ...
    public void tilt(Side side) {
        ...
        for (int col = 0; col < _board.size(); col += 1) {
            for (int row = _board.size() - 1; row > 0; row -= 1) {
                ...
                if (t != null) {
                    boolean merged = (merger(col, row, _board)); {
                    }
                }
            }
        }
        boolean moves = moves(side, _board);
    ...
    public boolean merger(int col, int row, Board b) {
        Tile t = b.tile(col, row);
        int counter = 0;
        for (int j = row - 1 ; j >= 0; j -= 1) {
                while (counter == 0) {
                    Tile x = tile(col, j);
                    if ((x != null) && (t.value() == x.value())) {
                        _score += t.value() * 2;
                        b.move(col, row, x);
                        counter += 1;
                        break;
    ...
    public boolean moves(Side side, Board board) {
        board.setViewingPerspective(side);
        boolean moveup = false;
        for (int col = 0; col < board.size(); col += 1) {
            for (int row = board.size()-1; row >= 0; row -= 1) {
                Tile t = board.tile(col, row);
                int counter = board.size()- row - 1;
                int rowcheck = 1;
                int count = 0;
                if (t!=null) {
                    while (count < counter && rowcheck <= counter) {
                        if (board.tile(col,row + rowcheck) == null) {
                            rowcheck += 1;
                            count += 1;
                        } else {
                            break;
    ...
    public static boolean atLeastOneMoveExists(Board b) {
        ...
        for (int row = 0; row < b.size()- 1 ; row += 1) {
            for (int col = 0; col < (b.size() -1); col += 1) {
                int adjrow = row + 1;
                int adjcol = col + 1;
                Tile r = b.tile(adjcol, row);
                Tile c = b.tile(col, adjrow);
                Tile x = b.tile(col, row);
                if (x.value() == c.value() || x.value() == r.value()) {
                    return true;
                }
            }
        }
